...
screen say(who, what):
    ...
    if discordrun:
        python:
            import os
            import io
            textthere = "{0}: {1}".format(who, what)
            status = state + "-" + day + "-" + st
            if who is not None and texthere != textthere:
                io.open("game/state.txt", "w+", encoding = "utf-8").write("{0}\n{1}".format(status, textthere))
                texthere = textthere
                whathere = what
            elif whathere != what:
                io.open("game/state.txt", "w+", encoding = "utf-8").write("{0}\n{1}".format(status, what))
                whathere = what
